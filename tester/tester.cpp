...
int main(int argc, char *argv[]){
	...
	try{
		...
		std::string bitmap_path = std::filesystem::temp_directory_path() / "fake-bitmap.bmp";
		...
		std::string sol_path = std::filesystem::temp_directory_path() / "sol.bmp";
		...
		for(std::int32_t k = 0; k < num_rows * num_cols; k++) {
		    ...
		    for(int di = -1; di <= 1; di++)
		        for(int dj = -1; dj <= 1; dj++) {
		        	int ni = i + di, nj = j + dj;
		            if(ni >= 0 and ni < num_rows and nj >= 0 and nj < num_rows) 
		...
		std::uint32_t remaining = 0;
		#ifdef __AVX512F__
			const __m512f threshold_vec = _mm512_set1_ps(error_threshold);
			for (std::uint32_t i = 0; i < file_size; i += 16) {
				__m512f abs_diff_vec = _mm512_abs_ps(_mm512_sub_ps(_mm512_load_ps(sol_data + i), _mm512_load_ps(student_data + i)));
			...
			for (std::uint32_t i = 0; i < file_size; i += 8) {
				__m256 diff_vec = _mm256_sub_ps(_mm256_load_ps(sol_data + i), _mm256_load_ps(student_data + i));
